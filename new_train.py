...
class Trainer:
    def __init__(self, model, epochs, early_stop=None,
                 ...
                 amp=True, device=None, _print=True):
    ...
    def _train_step(self, data, optimizer, scaler, scheduler):
        train_loss, metrics_train = self.train(data, optimizer, scaler, scheduler)
        val_loss, metrics_val, metrics_test, _ = self.evaluate(data)
...
class TrainerCLS(Trainer):
    def __init__(self, args, model,
                 ...
                 amp=False, device=None, _print=True, **kwargs):
        super().__init__(model=model, epochs=epochs, early_stop=early_stop, optim=optim, warnup=warnup, T_0=T_0, T_mult=T_mult, eta_min=eta_min,
                         amp=amp, device=device, _print=_print)
    ...
    def evaluate(self, data):
        ...
        return loss.detach().cpu().item(), metrics_val, metrics_test, out
...
class TrainerPrune(TrainerCLS):
    def __init__(self, args, model,
                 ...
                 amp=False, device=None, _print=False, **kwargs):
...
class TrainerRKD(TrainerCLS):
    def __init__(self, args, teacher, model,
                 epochs, momentum, updata=30, lr=0.001, weight_decay=0.0001, early_stop=None, scheduler=False,
    ...
    def evaluate(self, data):
        val_loss, metrics_val, metrics_test, self._last_out = super().evaluate(data)
        return val_loss, metrics_val, metrics_test, self._last_out
...
if __name__ == "__main__":
    ...
    gnn_type = 'sage'  # "gcn", "sage", "gin"
    hid_dim = 512
    num_layers = 3
    dropout = 0.7
    res = False
    norm = None  # "layer", None, "batch"
    ...
    fix_seed(123 if dataset in ["cora", "citeseer", "pubmed"] else seed)
    dataset, mask = load_dataset(dataset, split, label_num_per_class=20, missing_rate=missing_rate)
    if missing_rate > 0.0:
        dataset = filling_data(dataset, fill, alpha=apcfi_args[0], beta=apcfi_args[1])

    fix_seed(seed)
    start = perf_counter()
    model = MainModule(dataset.x.size(1), hid_dim, dataset.num_classes, num_layers, cugraph=cugraph,
                       dropout=dropout, res=res, norm=norm, gnn_type=gnn_type, gnn_cls=False)


    a = TrainerCLS(args=None, model=model,
                   epochs=epochs, lr=lr, weight_decay=weight_decay,
                   early_stop=mpp_early_stop, scheduler=scheduler,
                   optim=optim, warnup=0.1,
                   amp=amp, device=None, _print=True)


    a.fit(dataset)
    a.model.load_state_dict(a.best_state_dict)
    print(f"\nTraining time: {perf_counter() - start:.3f}s")
    check_float_and_time(a.model, dataset)

    fix_seed(seed)
    start = perf_counter()
    model2 = MainModule(dataset.x.size(1), hid_dim, dataset.num_classes, num_layers, cugraph=cugraph,
                        dropout=dropout, res=res, norm=norm, gnn_type=gnn_type, gnn_cls=False)

    mlp2 = mirror_projection(model2, gnn2mlp=True, cugraph=cugraph)
    c = TrainerPrune(args=None, model=mlp2,
                   epochs=mpp_epochs, lr=mpp_lr, weight_decay=mpp_weight_decay,
                   early_stop=mpp_early_stop, scheduler=mpp_scheduler,
                   mpp_load=mpp_load, mpp_step=mpp_step, prune_rate=prune_rate,
                   prune_type=prune_type, imp_type=imp_type, isomorphic=isomorphic, global_pruning=True,
                   optim=optim, warnup=0.1,
                   amp=amp, device=None, _print=False)

    c.fit(dataset)
    _sample_mlp = deepcopy(c.model)


    model_pruned = mirror_projection(c.model, gnn2mlp=False, cugraph=cugraph, load_state=mpp_load)
    fix_seed(seed)
    d = TrainerCLS(args=None, model=model_pruned,
                   epochs=epochs, lr=prune_lr, weight_decay=weight_decay,
                   early_stop=mpp_early_stop, scheduler=prune_scheduler,
                   optim=optim, warnup=0.1,
                   amp=False, device=None, _print=True)

    d.fit(dataset)
    d.model.load_state_dict(d.best_state_dict)
    check_float_and_time(d.model, dataset)

    fix_seed(seed)

    if not mpp:
        print("Using: full gnn")
        sample_mlp = a.model
        sample_mlp.load_state_dict(a.best_state_dict)
    else:
        if kd_mpnn_mlp:
            print("Using: prune gnnMLP")
            _sample_mlp.reset_parameters()
            sample_mlp = _sample_mlp
        else:
            print("Using: MLP")
            sample_mlp = MainModule(dataset.x.size(1), 512, dataset.num_classes, 1, cugraph=cugraph,
                       dropout=dropout, res=True, norm=norm, gnn_type="linear", gnn_cls=False, act="relu")

    e =  TrainerRKD(args=None, teacher=d.model, model=sample_mlp, lamb=kd_lamb,
                   epochs=kd_epochs, lr=kd_lr, weight_decay=kd_weight_decay,
                   early_stop=mpp_early_stop, scheduler=kd_scheduler,
                   optim=optim, warnup=0.1,
                   amp=amp, device=None, _print=True)

    e.fit(dataset, kd_smoothing)
    e.model.load_state_dict(e.best_state_dict)
    print(f"\nTraining time: {perf_counter() - start:.3f}s")
    check_float_and_time(e.model, dataset)
    print(e.model.layers)
