...
def get_conv(gnn_type, in_channels, out_channels,
    ...
    if 'gcn' in _gnn_type:
        ...
        if cugraph and CuGraphSAGEConv is not None:
            conv = CuGraphGATConv(in_channels, out_channels//4, heads=4, concat=True, bias=False)
        else:
            conv = GATConv(in_channels, out_channels//4, heads=4, concat=True,
                           add_self_loops=False, bias=False)

        if 'mlp' in _gnn_type:
            conv = GATConvMLP(conv=conv)
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'layers'):
            for layer in self.layers:
                ...
                if layer is not None and isinstance(layer, (CuGraphSAGEConv, CuGraphGATConv)):
                    self._CuGraph = True
    ...
    def forward(self, x, edge_index=None, **kwargs):
        ...
        if hasattr(self, 'layers'):
            for idx, layer in enumerate(self.layers):
                _input = (x, edge_index) if self._use_edges[idx] else (x,)
                if self.res:
                    _x = layer(*_input) + self.res_layers[idx](x)
                else:
                    _x = layer(*_input)
                ...
                if hasattr(self, "norms"):
                    _x = self.norms[idx](_x)

                if not self.act_first:
                    _x = self.acts[idx](_x)

                if not ("MLP" in layer.__class__.__name__ or isinstance(layer, nn.Linear)):
                    _x = F.dropout(_x, p=self.dropout, training=self.training)
                x = _x

        if self._use_edges[-1]:
            x_out = self.cls_conv(x, edge_index)
        else:
            x_out = self.cls_conv(x)
        return x_out
...
def projection_layer(conv, gnn2mlp=True, cugraph=False, debug=False):
    ...
    if "MLP" in _name:
        ...
        if "sage" in _name.lower():
            ...
            _name = "GATConv"
            if cugraph and CuGraphGATConv is not None:
                _name = "CuGraphGATConv"
                _params = {k : v for k, v in _params.items()
                           if k in get_func_args(CuGraphGATConv.__init__)}
...
if __name__ == "__main__":
    ...
    a = MainModule(in_dim, 256, 30, 6
                   ,dropout=0.6, res=True, norm='layer',gnn_type='gat', gnn_cls=True, cugraph=False)
    print(a.layers)
    print(a._CuGraph)

    b = mirror_projection(a, True, )

    Edge_Index = EdgeIndex(edge_index) if a._CuGraph else edge_index
    a = a.cuda()
    print(a.layers)
    print(b.layers)
    print(a(x.cuda(), Edge_Index.cuda()).shape)
