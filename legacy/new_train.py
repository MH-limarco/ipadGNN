...
class Trainer:
    ...
    def _fit(self, data, lr, weight_decay, scheduler):
        ...
        if self.model._CuGraph:
            data.edge_index = EdgeIndex(data.edge_index)

        torch.cuda.reset_peak_memory_stats(self.device if self.device != "cpu" else None)
        ...
        max_vram = torch.cuda.max_memory_reserved(self.device if self.device != "cpu" else None) / 1024 ** 2
        if self._print:
    ...
    def _get_scheduler(self, optimizer):
        ...
        if warmup_epochs is not None and 1.0 >= warmup_epochs >= 0:
...
def read_config(data_name, dir, config_root="./"):
    import yaml, os
...
def read_best_config(data_name, model_type, prune_rate=0.0):
    gnn_dir = "new_best_prune_cfg" if prune_rate > 0 else "new_best_cfg"
...
if __name__ == "__main__":
    ...
    model = MainModule(dataset.x.size(1), config["hidden"], dataset.num_classes, config["num_layers"],
    ...
    if prune_rate > 0.0:
        prune_trainer = TrainerPrune(args=config, model=model,
                                     epochs=config["prune_epochs"], lr=config["prune_lr"], weight_decay=config["prune_weight_decay"],
                                     prune_rate=prune_rate, mpp_step=config["prune_step"],
                                     prune_type=config["pruner_type"], imp_type=config["imp_type"],
                                     isomorphic=config["isomorphic"],
                                     scheduler=False, warmup=0.0,
                                     early_stop=-1, amp=True, gpu_idx=0, _print=True)

        prune_trainer.fit(dataset)
        pruned_gnn = mirror_projection(prune_trainer.model, gnn2mlp=False)
