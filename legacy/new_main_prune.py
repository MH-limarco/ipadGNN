...
def experiment_loop(seed, dataset, split,
    ...
    for idx in range(runs):
        best_acc = experiment_step(idx, seed, dataset, optim,
                    mpp, prune_mlp,
                    gnn_type, hidden, num_layers, dropout, residual, norm,
                    epochs, lr, weight_decay, scheduler, warnup,
...
def experiment_step(run_idx, seed, dataset, optim,
    ...
    return gnn_trainer._best_metrics_test
...
def _get_gnn_suggest(trial, mpp=True):
    ...
    if mpp:
        ...
        mpp_epochs = 150 * prune_step

        prune_rate = trial.suggest_float("prune_rate", 0.5, 0.9, step=0.10)
...
def optuna_objective(trial, dataset_name, model_name, runs):
    ...
    cugraph = True if dataset_name.lower() in ["cs", "physics", "wikics"] else False
    avg_acc = experiment_main(dataset_name, model_name,
                                        **kwargs,
                                        runs=runs, amp=True, mpnn_amp=False, cugraph=cugraph,
                                        optim="Adam",

                                        )

    print(f"\nTrial {trial.number} finished with {kwargs['prune_rate']:.2f}-prune gnn accuracy: {avg_acc:.4f}\n")
    avg_acc = avg_acc if np.isfinite(avg_acc) else 0.0
    return avg_acc, kwargs["prune_rate"]

def optuna_optimize(dataset_name, gnn_type, runs=3, n_trials=50):
    import os
    root = "new_optuna_pruner"
    ...
    pareto_df.to_csv(f"./{root}/csv/{dataset_name.lower()}/{gnn_type.lower()}_{n_trials}_pareto.csv", index=False)
...
if __name__ == "__main__":
    for dataset in ["cora", "citeseer", "pubmed", "photo", "computers", "wikics", "cs", "physics"]:#[::-1]:
        for conv_name in ["sage", "gcn"]:
            #conv_name = "gcn"
            optuna_optimize(dataset_name=dataset, gnn_type=conv_name, runs=3,
                            n_trials=100)
