...
def get_conv(gnn_type, in_channels, out_channels,
             dropout=0.5, normalize=True, train_eps=False,
             cugraph=False):
    ...
    def reset_parameters(self):
        if hasattr(self, 'layers'):
            for layer in self.layers:
                ...
                if isinstance(layer, CuGraphSAGEConv):
                    self._CuGraph = True
...
def mirror_projection(module, gnn2mlp=True, cugraph=False, mirror_attr=None, load_state=False):
    ...
    state = module.state_dict() if load_state else None
    for attr in mirror_attr:
        if hasattr(module, attr):
            _module = getattr(module, attr)
            if isinstance(_module, nn.ModuleList):
                ...
                setattr(module, attr, projection_layer(_module, gnn2mlp, cugraph))
...
if __name__ == "__main__":
    a = MainModule(1024, 256, 30, 6
                   ,dropout=0.6, res=True, norm='layer',gnn_type='sage_mlp', gnn_cls=True)
