...
from engine.moudle.model import LimelightModule
...
if __name__ == "__main__":
    ...
    if best:
        config = read_best_config(dataname, gnn_type, prune_rate)

    else:
        config = {"seed": 42, "hidden": 512, "num_layers": 2, "dropout": 0.5,
                  "norm": "layer", "residual": False}
    ...
    model = LimelightModule(dataset.x.size(1), config["hidden"], dataset.num_classes, config["num_layers"],
                       dropout=config["dropout"], res=config["residual"], norm=config["norm"], gnn_type=_gnn_type,
                       gnn_cls=False)
    ...
    for run in range(3):
        ...
        if prune_rate > 0.0:
            i_mlp_path = f"imlp_seed_{_seed}_{dataname}_{gnn_type}_{prune_rate}.pt"
            if os.path.exists(i_mlp_path):
                prune_mlp = LimelightModule(pt=i_mlp_path)
            else:
                prune_trainer = TrainerPrune(args=config, model=model,
                                             epochs=300, lr=0.00055,
                                             weight_decay=2.37e-05,
                                             prune_rate=0.6, mpp_step=2,
                                             prune_type="group", imp_type="group",
                                             isomorphic=False,
                                             scheduler=True, warmup=0.1, T_0= 30,
                                             early_stop=-1, amp=True, gpu_idx=0, _print=True)

                prune_trainer.fit(dataset)

                prune_trainer.model.save(i_mlp_path, True)
                prune_mlp = prune_trainer.model

            pruned_gnn = mirror_projection(prune_mlp, target="gnn")

            gnn_path = f"prunedgnn_seed_{_seed}_{dataname}_{gnn_type}_{prune_rate}.pt"

            if os.path.exists(gnn_path):
                teacher_gnn = LimelightModule(pt=gnn_path)
            else:
                gnn_trainer = TrainerCLS(args=None, model=pruned_gnn,
                                         epochs=500, lr=0.0038, weight_decay=8.2e-05,
                                         early_stop=-1, optim="Adam", gpu_idx=0,
                                         T_0= 60, scheduler=True, warmup=0.05,
                                         amp=False, device=None, _print=True)

                gnn_trainer.fit(dataset)
                gnn_trainer.model.save(gnn_path, True)
                teacher_gnn = gnn_trainer.model
                #

        else:
            gnn_path = f"gnn_seed_{_seed}_{dataname}_{gnn_type}.pt"
            if os.path.exists(gnn_path):
                teacher_gnn = LimelightModule(pt=gnn_path)
        ...
        if kdmlp:
            mlp = LimelightModule(dataset.x.size(1), 512, dataset.num_classes, 1,
                                  dropout=0.5, res=False, norm="batch", gnn_type=_gnn_type,
                                  gnn_cls=False)

            imlp = LimelightModule(pt=i_mlp_path)
            #imlp.reset_parameters()

            student = imlp #if prune_rate > 0.0 else mlp

            kd_path = f"kd_seed_{_seed}_{dataname}_{'imlp' if prune_rate > 0.0 else 'mlp'}.pt"
            fix_seed(_seed)
            kd_trainer = TrainerRKD(args=None, teacher=teacher_gnn, student=student,
                                    epochs=1000, momentum=0.995, lr=0.00032, weight_decay=0.075,
                                    lamb = 1.2, alpha=0.25, warmup= 0.05, scheduler=False,
                                    feat_loss="cwd", temperature=2.0,
                                    early_stop=-1, optim="Adam", gpu_idx=0, label_smoothing=0.25,
                                    amp=True, device=None, _print=True)

            kd_trainer.fit(dataset)
            metrics_train, metrics_val, metrics_test = kd_trainer.test(dataset)
            acc.append(metrics_test)
            #kd_trainer.model.save(kd_path, True)
    torch.cuda.empty_cache()
    gc.collect()
    print(f"Mean: {sum(acc) / len(acc):.4f}, Std: {torch.std(torch.tensor(acc)):.4f}")
