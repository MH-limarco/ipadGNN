...
from engine.utils import _get_flops

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
AMP_TYPE = torch.bfloat16 #torch.float16
...
def prune(model, dataset, prune_rate, pruner_type, imp_type, isomorphic, global_pruning=True, device=None):
    ...
    base_FLOPS = _get_flops(model, dataset)
    pruner.step()
    FLOPS = _get_flops(model, dataset)
    model._config_updated = False
    print(f"FLOPS: {base_FLOPS} -> {FLOPS}")
    return model
...
class Trainer:
    def __init__(self, model, epochs, early_stop=None, label_smoothing=None,
                 ...
                 amp=True, device=None, gpu_idx=None, _print=True, filler=None):
        self._print = _print
    ...
    def _fit(self, data, lr, weight_decay):
        ...
        for epoch in self.qbar:
            if hasattr(self.model, "start_global"):
                _start_global = self.model.start_global(epoch)
                if _start_global:
                    self._patience = 0
    ...
    def evaluate(self, data, loss_flag):
        self.model.eval()
        with torch.no_grad(), torch.autocast(device_type=self.device, enabled=self.amp, dtype=AMP_TYPE):  #
...
class TrainerCLS(Trainer):
    def __init__(self, args, model,
                 ...
                 amp=False, device=None, gpu_idx=0, _print=True, filler=None, **kwargs):
        super().__init__(model=model, epochs=epochs, scheduler=scheduler, early_stop=early_stop, optim=optim, 
                         ...
                         label_smoothing=label_smoothing, amp=amp, device=device, gpu_idx=gpu_idx, _print=_print, filler=filler)
...
class TrainerPrune(TrainerCLS):
    def __init__(self, args, model,
        ...
        super().__init__(args=args, model=model, epochs=round(epochs//mpp_step), lr=lr, weight_decay=weight_decay,
                         ...
                         T_0=T_0, T_mult=T_mult, eta_min=eta_min, amp=amp, device=device, _print=_print, filler=filler)
...
class TrainerRKD(TrainerCLS):
    def __init__(self, args, teacher, student,
                 ...
                 alpha=0.0, label_smoothing=None, amp=False, device=None, gpu_idx=0, _print=True, krd=True, filler=None, **kwargs):
        super().__init__(args=args, model=student, epochs=epochs, lr=lr, weight_decay=weight_decay,
                         ...
                         label_smoothing=label_smoothing, gpu_idx=gpu_idx, _print=_print, filler=filler)
        self.krd = krd
        self.lamb = lamb
...
if __name__ == "__main__":
    ...
    config = {"seed": 42, "hidden": 32, "local_layers": 5, "global_layers": 4, "heads": 8,
              "dropout": 0.5, "in_dropout": 0.1}
    ...
    gnn_trainer = TrainerCLS(args=None, model=model,
                             epochs=1600, lr=0.001, weight_decay=5e-4,
