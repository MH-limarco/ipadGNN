...
class Trainer:
    ...
    def _fit(self, data, lr, weight_decay):
        ...
        for epoch in self.qbar:
            if epoch == 200 and hasattr(self.model, "start_global"):
                self.model.start_global()
...
if __name__ == "__main__":
    ...
    split = "fixed" if dataname.lower() not in ["cora", "citeseer", "pubmed"] else "class"

    config = {"seed": 42, "hidden": 128, "layers": 3, "num_heads": 1,
              "dropout": 0.7}

    fix_seed(config["seed"])
    dataset, _ = load_dataset(dataname, split, missing_rate=0.0, label_num_per_class=20)
    #dataset = filling_data(dataset, "zero")

    gnn = LimelightModule(dataset.x.size(1), config["hidden"], config["hidden"], config["layers"],
                       dropout=config["dropout"], res=False, norm='batch', gnn_type="gcn",
                       gnn_cls=True)


    model = SGFormer(in_channels=dataset.x.size(1), hidden_channels=config["hidden"], out_channels=dataset.num_classes,
                     num_layers=config["layers"], dropout=config["dropout"],
                     graph_weight=0.8, gnn=gnn, num_heads=config["num_heads"])


    #model = P(model)
    gnn_trainer = TrainerCLS(args=None, model=model,
                             epochs=1000, lr=0.001, weight_decay=0.0,
                             early_stop=500, optim="Adam", gpu_idx=0,
                             T_0=1, scheduler=False, warmup=0.00,
                             amp=False, device=None, _print=True)

    gnn_trainer.fit(dataset)
    print(gnn_trainer.model)
