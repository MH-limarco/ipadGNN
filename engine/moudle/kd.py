...
class ReliableProbModel(torch.nn.Module):
    ...
    def initialization(self, teacher, data):
        self.ready = False
        if hasattr(teacher, "_CuGraph") and teacher._CuGraph:
        ...
        logit_teacher = teacher(data.x, data.edge_index)
        logit_teacher_noise = teacher(noise_data.x, noise_data.edge_index)
        entropy_org = Categorical(logits=logit_teacher.softmax(dim=-1).detach()).entropy()
        entropy_noise = Categorical(logits=logit_teacher_noise.softmax(dim=-1).detach()).entropy()

        weight = torch.abs(entropy_noise - entropy_org)
        self.delta_entropy = weight / torch.max(weight)
        self.pred_teacher = torch.argmax(logit_teacher, dim=1)
        self.logit_teacher = logit_teacher
