...
EDGE_MISSING_DIR = 'missing_exp/edge_missing'
...
def get_missing_rate(fname):
    m = re.search(r'edge[_\-]?([0-9.]+)', fname)
    if m:
...
for dataset in DATASETS:
        ...
        for file in files:
            if file.endswith('.txt') and dataset in file:
                ...
                if model_name not in data_dict:
                    data_dict[model_name] = [np.nan, np.nan, np.nan, np.nan]
    ...
    os.makedirs(f"_csv/edge_missing", exist_ok=True)
    df.to_csv(f"./_csv/edge_missing/{dataset}.csv", index=False)
