...
MISSING_DIR = f'./missing_exp/{missing_type}_missing'
...
def get_imp_type(path, fname):

    for imp in _imp:
        if imp in path or imp in fname:
            return imp


for imp_type in _imp:
    print(imp_type)
    for dataset in DATASETS:
        ...
        for root, dirs, files in os.walk(MISSING_DIR):
            for file in files:
                if file.endswith('.txt') and f"_{dataset}" in file:
                    fpath = os.path.join(root, file)
                    missing_rate = get_missing_rate(file)
                    file_imp_type = get_imp_type(root, file)
                    if file_imp_type != imp_type:
                        continue
        ...
        os.makedirs(f"./_csv/{CSV_DIR}/dataset", exist_ok=True)
        df.to_csv(f"./_csv/{CSV_DIR}/dataset/{imp_type}_{dataset}.csv", index=False)
