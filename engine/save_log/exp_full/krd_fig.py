...
hist = hist_true / (hist_true + hist_false + 1e-5)  # 正規化直方圖
...
plt.xlabel('Normalized Entropy Change (Δe)')
plt.ylabel('Sample Count')
plt.title('Teacher-Student Match\n(hist_true)')
plt.tight_layout()
plt.savefig(f'./_fig/krd_hist_true_{true_power:.1f}.png')
plt.show()
...
plt.tight_layout()
plt.savefig(f'./_fig/krd_hist_false_{true_power:.1f}.png')
plt.show()
...
plt.xlabel('Normalized Entropy Change (Δe)')
plt.ylabel('Empirical Probability (p)')
plt.title('MP-KRD Curriculum: Empirical Bar Plot & Fitted Power Curve\n(hist_fin)')
plt.ylim(0, 1.00)
plt.legend()
plt.tight_layout()
plt.savefig(f'./_fig/krd_hist_{true_power:.1f}.png')
plt.show()
