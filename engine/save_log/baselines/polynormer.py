...
class GlobalAttn(torch.nn.Module):
    prunable = ["h_lins", "q_lins", "k_lins", "v_lins"]
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            h = self.h_lins[i](x)
            h = self.k_lins[i](h)
            k = F.sigmoid(h).view(seq_len, -1, self.heads)
            if self.qk_shared:
                q = k
            else:
                q = F.sigmoid(self.q_lins[i](x)).view(seq_len, -1, self.heads)
            v = self.v_lins[i](x).view(seq_len, -1, self.heads)
...
class Polynormer(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, local_layers=3, global_layers=2,
            in_dropout=0.15, dropout=0.5, global_dropout=0.5, heads=1, beta=-1, g_epochs=200, pre_ln=False):
    ...
    def start_global(self, epoch):
        """Start global attention."""
        if self.g_epochs == epoch:
            print("Global attention started.")
            self._global = True
            return True
        return False

    def forward_with_hidden(self, x, edge_index):
        hidden = []
        x = F.dropout(x, p=self.in_drop, training=self.training)
        x = self.lin_in(x)
        hidden.append(x)
        ...
        ## equivariant global attention
        if self._global:
            x_global = self.ln(x_local)
            hidden.append(x_global)
            x_global, _x_global_hidden = self.global_attn.forward_with_hidden(x_global)
            hidden.extend(_x_global_hidden)
