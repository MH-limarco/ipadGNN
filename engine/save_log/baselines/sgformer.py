...
class TransConvLayer(nn.Module):
    ...
    def forward(self, query_input, source_input, edge_index=None, edge_weight=None, output_attn=False):
        ...
        seq_len, _ = query_input.size()
        query = self.Wq(query_input).reshape(seq_len,
                                             self.num_heads, -1)
        key = self.Wk(source_input).reshape(seq_len,
                                            self.num_heads, -1)
        if self.use_weight:
            value = self.Wv(source_input).reshape(seq_len,
                                                  self.num_heads, -1)
        else:
            value = source_input.reshape(seq_len, 1, -1)
...
class SGFormer(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        if self.use_graph:
            ...
            if self.aggregate == 'add':
                ...
                x = torch.cat((x1, self.gnn_out(x2, edge_index)), dim=1)
        else:
            x = x1
        x = self.fc(x)
        return x
    ...
    def forward_with_hidden(self, x, edge_index):
        hidden = []
        x1, _hidden_x1 = self.trans_conv.forward_with_hidden(x, edge_index)
        hidden.extend(_hidden_x1)
        hidden.append(x1)

        if self.use_graph:
            x2, _hidden_x2 = self.gnn.forward_with_hidden(x, edge_index)
            hidden.extend(_hidden_x2)
            hidden.append(x2)
            if self.aggregate == 'add':
                x = self.graph_weight * x2 + (1 - self.graph_weight) * x1
            else:
