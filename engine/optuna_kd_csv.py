...
sorted_keys = ["prune_rate", "momentum", "loss_type",
...
def _read_df(dir_roots, data_name, gnn_type, runs=125):
    ...
    if "params_feat_loss" in df.columns:
        df_sorted["params_feat_loss"] = df_sorted["params_feat_loss"].fillna("cwd")
    return df_sorted
...
def build_ikd_yaml(data_name, model, runs=125):
    import yaml
    roots = ["optuna_kd_feat", "optuna_kd_cwd_feat"]
    yaml_root = "./best_config"

    cfg_path = os.path.join(yaml_root, "gnns", "ikd_cfg")
