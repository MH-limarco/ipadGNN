...
def experiment_main(data_name, gnn_type,
                    ...
                    mpp_epochs, mpp_lr, mpp_weight_decay, mpp_scheduler, prune_warnup,
    ...
    return experiment_loop(scheduler=scheduler, warnup=warnup, mpp=mpp, prune_mlp=prune_mlp,
                           ...
                           mpp_epochs=mpp_epochs, mpp_lr=mpp_lr, mpp_weight_decay=mpp_weight_decay, mpp_scheduler=mpp_scheduler, prune_warnup=prune_warnup,
...
def experiment_step(run_idx, seed, dataset, optim,
    ...
    if mpp:
        ...
        prune_trainer = TrainerPrune(args=None, model=gnnmlp,
                         ...
                         optim=optim, warnup=prune_warnup,
        ...
        gnn_trainer = TrainerCLS(args=None, model=model_pruned,
                       ...
                       amp=mpnn_amp, device=None, _print=0)
    ...
    best_acc = RKD_trainer._best_metrics_test
...
def optuna_objective(trial, dataset_name, model_name, runs):
    ...
    if mpp:
        ...
        prune_step = trial.suggest_int("prune_step", 1, 5)
        prune_rate = trial.suggest_float("prune_rate", 0.5, 0.9, step=0.05)
        ...
        mpp_weight_decay_power = trial.suggest_int("mpp_weight_decay_power", 0, 5, step=1)
        mpp_weight_decay = 10 ** (-mpp_weight_decay_power)

        mpp_lr_power = trial.suggest_int("mpp_lr_power", 2, 5, step=1)
        mpp_lr = 10 ** (-mpp_lr_power)
    ...
    kd_lamb = trial.suggest_float("kd_lamb", 0.0, 0.9, step=0.05)
    kd_momentum = trial.suggest_categorical("kd_momentum", [0.995, 0.99, 0.9])
    kd_lr_power = trial.suggest_int("kd_lr_power", 2, 6, step=1)
    kd_lr = 10 ** (-kd_lr_power)
    kd_weight_decay_power = trial.suggest_int("kd_weight_decay_power", 0, 5, step=1)
    kd_weight_decay = 10 ** (-kd_weight_decay_power)
    ...
    avg_acc, avg_time = experiment_main(dataset_name, model_name,
                                        scheduler=scheduler, warnup=warnup,
                                        mpp=mpp, prune_mlp=prune_mlp, missing_rate=missing_rate,
                                        prune_step=prune_step, prune_rate=prune_rate,
                                        prune_type=prune_type, imp_type=imp_type, isomorphic=isomorphic,
                                        mpp_epochs=mpp_epochs, mpp_lr=mpp_lr,
                                        mpp_weight_decay=mpp_weight_decay,
                                        mpp_scheduler=mpp_scheduler, prune_warnup=mpp_warnup,

                                        kd_dropout=kd_dropout, kd_residual=kd_residual, kd_norm=kd_norm,
                                        kd_epochs=kd_epochs, kd_lr=kd_lr, kd_weight_decay=kd_weight_decay,
                                        kd_scheduler=kd_scheduler, kd_warnup=kd_warnup,
                                        kd_lamb=kd_lamb, kd_momentum=kd_momentum, kd_smoothing=kd_smoothing,
...
if __name__ == "__main__":
    optuna_optimize(dataset_name="cora", gnn_type="gcn", n_trials=1)
